...
page=$(curl -s "$URL")
check "page serves 200"            200 "$(curl -s -o /dev/null -w '%{http_code}' "$URL")"
...
check "unauthenticated read denied"   403 "$(curl -s -o /dev/null -w '%{http_code}' "$FS/dashboard/main")"
check "unauthenticated write denied"  403 "$(curl -s -o /dev/null -w '%{http_code}' -X PATCH -H 'Content-Type: application/json' -d '{"fields":{"x":{"stringValue":"y"}}}' "$FS/dashboard/main")"
check "other collections denied"      403 "$(curl -s -o /dev/null -w '%{http_code}' "$FS/anything/doc1")"
